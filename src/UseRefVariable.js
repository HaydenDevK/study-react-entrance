/*
    useRef로 '컴포넌트 안에서 조회 및 수정이 가능한 변수를 관리하고 싶을 때'의 예제이다.
    그런 경우가 언제일까? 감을 잡아보자.

    자, 먼저 리액트는 특정 조건이 발생할 때 마다 렌더링을 발생시킨다.
        - setState 함수로 state가 변경되었을 때
        - 부모 컴포넌트가 렌더링될 떼
        - 부모 컴포넌트에서 받는 props가 변경되었을 때
        - shouldComponentUpdate에서 true가 반환될 때
        - forceUpdate 메서드가 실행될 때
    그리고, 렌더링은 다음의 순서로 진행된다.
        - render : React.createElement로 생성
        - reconciliation : 이전 elements와 새로 생성된 elements 비교
        - commit : (reconciliation에서 변경 사항이 감지되면) DOM update
    그리고 이 과정이 모두 끝나야? 변경된 값이 조회된다. (그 전까지는 변경 전 값이 조회된다.)
    그리고 리렌더링이 발생할 때 마다, let var const 변수 선언문도 실행되기 때문에 변수 값이 초기화된다.
        - 함수 컴포넌트 내부에서 let nextId = 4; 이렇게 변수를 선언하고 할당하면,
        -> 리렌더링이 발생할 때마다 매번 선언문이 실행되서 매번 값이 초기화된다.
    
    그에 비해,
    1. useRef는 값이 변경되어도 리렌더링을 발생시키지 않는다.
        useRef는 순수 자바스크립트 객체로, heap 공간에 저장되어 어플리케이션이 종료되거나 가비지 컬렉팅될 때 까지, 참조할 때 마다 같은 메모리 주소를 가진다.
        -> 계속 같은 주소를 가지기 때문에 === 연산 시 항상 true를 반환한다.
        -> 그래서 실제 값이 변경되도, 변경 여부가 감지되지 않아서 리렌더링을 발생시키지 않는다.
        -> 이런 특성을 활용해서 : 바뀔 때 마다 리렌더링될 필요가 없고, 오히려 불필요한 리렌더링을 막고 싶을 수 있을 때 useRef를 활용한다.
    2. useRef는 current 값이 heap에 저장되어, 컴포넌트가 (다른 변경 사항으로) 리렌더링 되어도 그 값을 가지고 있다.
        - 즉, 리렌더링이 발생하더라도, 최종 값을 저장하고 있어서 초기화되지 않는다.

    * 요약
        - useRef의 특성
            - 리렌더링을 발생시키지 않는다.
            - 그 때문에 변경 후 즉시 조회 가능하다.
            - 그리고 (다른 이유로) 리렌더링이 발생하더라도, 초기화되지 않고 마지막 값을 유지한다는 특징도 있다.
        - useRef의 용도
            - setTimeout, setInterval을 통해 만들어진 id
            - scroll의 위치
            - 배열에 새 항목이 추가 될 때 필요한 고유한 key 값 (주로 id 값)
    ...
    return (
        <div>
            ...
            <h3>- 사전 지식</h3>
                ...
                그리고 리렌더링이 발생할 때 마다, let var const 변수 선언문도 실행되기 때문에 변수 값이 초기화된다.<br/>
                    <blockquote>- 함수 컴포넌트 내부에서 let nextId = 4; 이렇게 변수를 선언하고 할당하면,</blockquote>
                    <blockquote>&raquo; 리렌더링이 발생할 때마다 매번 선언문이 실행되서 매번 값이 초기화된다.</blockquote>
                <br/>
                그에 비해,<br/>
                1. useRef는 값이 변경되어도 리렌더링을 발생시키지 않는다.<br/>
                    ...
                    <blockquote>&raquo; 계속 같은 주소를 가지기 때문에 === 연산 시 항상 true를 반환한다.</blockquote>
                    <blockquote>&raquo; 그래서 실제 값이 변경되도, 변경 여부가 감지되지 않아서 리렌더링을 발생시키지 않는다.</blockquote>
                    <blockquote>&raquo; 이런 특성을 활용해서 : 바뀔 때 마다 리렌더링될 필요가 없고, 오히려 불필요한 리렌더링을 막고 싶을 수 있을 때 useRef를 활용한다.</blockquote>
            ...
            <h3>- 예제</h3>
            <h4>countRef.current : {countRef.current}</h4>
